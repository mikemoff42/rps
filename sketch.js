...
function createSliders(){
  ...
  slider_speed = createSlider(1,10,5,1);  
...
function generateRPS(){
  ...
  for (let i=0;i<NumberOfEach;i++) 
    rocks[i] = new Rock(random(loc_min,loc_max),random(loc_min,loc_max),random(-Speed,Speed),random(-Speed,Speed));
  for (let i=0;i<NumberOfEach;i++) 
    papers[i] = new Paper(random(loc_min,loc_max),random(loc_min,loc_max),random(-Speed,Speed),random(-Speed,Speed));
  for (let i=0;i<NumberOfEach;i++) 
    scissors[i] = new Scissor(random(loc_min,loc_max),random(loc_min,loc_max),random(-Speed,Speed),random(-Speed,Speed));
  ...
  for (let i=rocks.length-1;i>=0;i--){
    for (let j=papers.length-1;j>=0;j--){
      if(rocks[i].collide(papers[j]) && !rocks[i].toDelete){
        let p = new Paper(rocks[i].x,rocks[i].y,rocks[i].xspeed,rocks[i].yspeed);
        p.collided = true;
        papers.push(p);
    ...
    for (let k=scissors.length-1;k>=0;k--){
      if(rocks[i].collide(scissors[k]) && !scissors[k].toDelete){
        let r = new Rock(scissors[k].x,scissors[k].y,scissors[k].xspeed,scissors[k].yspeed)
        r.collided = true;
        rocks.push(r);
  ...
  for (let i=scissors.length-1;i>=0;i--){
    for (let j=papers.length-1;j>=0;j--){
      if(scissors[i].collide(papers[j]) && !papers[j].toDelete){
        let s = new Scissor(papers[j].x,papers[j].y,papers[j].xspeed,papers[j].yspeed);
        s.collided = true;
        scissors.push(s);
